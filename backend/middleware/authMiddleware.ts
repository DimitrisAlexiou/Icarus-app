...
import { UserProps, getUserById } from '../models/users/user';
...
export const authorize = async (
	...
	if (
		...
		try {
			...
			req.user = (await getUserById(decoded.id).select(
				'-password'
			)) as UserProps;
...
export const checkUserRole = (allowedRoles: string[]) => {
	return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
		...
		if (!allowedRoles.includes(req.user.type))
			return res
				.status(403)
				.json({ message: 'Forbidden! Cannot access this resource.' });
...
export const isOwner = async (
	...
	try {
		...
		if (currentUserId.toString() !== userId)
			throw new CustomError('Forbidden! Cannot access this resource.', 403);
		// return res
		// 	.status(403)
		// 	.json({ message: 'Forbidden! Cannot access this resource.' });
