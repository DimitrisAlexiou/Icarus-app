...
export const register = tryCatch(async (req: Request, res: Response) => {
	...
	if (!name || !surname || !username || !email || !password) {
		if (type === 'Student' && (!studentId || !entranceYear || !studentType)) {
			throw new CustomError('Please fill in all the student required fields.', 400);
		} else if (type === 'Instructor' && !facultyType) {
...
// export const registerRefactor = tryCatch(async (req: Request, res: Response) => {
// 	const {
// 		name,
// 		surname,
// 		username,
...
// 		const usernameTaken = await User.findOne({ username }).session(session);
// 		if (usernameTaken) {
...
// 		return res.status(201).json({ user: newUser, token: generateToken({ id: user._id }) });
// 	} catch (error) {
...
// 		return res.status(500).json({
// 			message: 'Something went wrong, unfortunately the account was not created.',
// 		});
// 	}
// });
